...
DOCKER_SETUP="--cap-add=NET_ADMIN  --shm-size=2g --rm -v "$(pwd)"/config:/config -v "$(pwd)":/sitespeed.io -v /etc/localtime:/etc/localtime:ro -e MAX_OLD_SPACE_SIZE=3072 "
  ...
  for browser in "${BROWSERS[@]}"
    do
      while read url; do
        ...
        docker run $DOCKER_SETUP $DOCKER_CONTAINER $NAMESPACE $CONFIG/desktop.json -b $browser $url --graphite.host 192.168.0.187
        control
      done <$file
    done
done
